...
static bool last_right_connected;

static void update_battery_label(struct split_battery_state state) {
    char text[16];

    if (state.right_connected) {
        snprintf(text, sizeof(text), "L%u R%u", state.left, state.right);
    } else {
        snprintf(text, sizeof(text), "L%u R--", state.left);
    }
...
static struct split_battery_state get_battery_state(const zmk_event_t *eh) {
    struct split_battery_state state = {
        .left = zmk_battery_state_of_charge(),
        .right = last_right,
        .right_connected = last_right_connected,
    };
    ...
    if (remote != NULL) {
        last_right = remote->state_of_charge;
        last_right_connected = true;
...
ZMK_DISPLAY_WIDGET_LISTENER(split_battery_status, struct split_battery_state, update_battery_label,
                            get_battery_state)
